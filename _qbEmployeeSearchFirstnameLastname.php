...
header('Access-Control-Allow-Credentials: true');
    ...
    $EXPECTED = array('token',
        'jwt','private_key','GivenName','FamilyName');
...
if(!$isAuth){
    ...
    if(count($token_info)>0){
        ...
        $rsl = $ObjectVC->qbEmployeeSearch($dataService,$GivenName,$FamilyName);

        if(is_numeric($rsl["employeeID"]) && !empty($rsl["employeeID"])){
            $ret = array('ERROR'=>'','employeeID'=>$rsl["employeeID"]);
        }else{
            $ret = array('ERROR'=>$rsl["TheStatusCodeIs"],'TheHelperMessageIs'=>$rsl["TheHelperMessageIs"],
                'TheResponseMessageIs'=>$rsl["TheResponseMessageIs"],'employeeID'=>'');
        }
    }
