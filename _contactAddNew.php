    ...
    if($isAuth['AUTH']){
        ...
        if(!$errObj['error']){
            ...
            if(is_numeric($result["ID"]) && !empty($result["ID"])){
                ...
                if(is_numeric($p) && empty($quickbook_id)){
                    $SSN="";
                ...
                }elseif(is_numeric($p)&& is_numeric($quickbook_id)){
                    $rsl_emp= $Object->updateQBEmployeeID_contactID($result["ID"],$quickbook_id);
                }
                //vendor test
                $rsl_vendor='';
                $p1= stripos($contact_type,"Vendor");
                if(is_numeric($p1)&& empty($quickbook_id)){
                    $data = array('Line1'=>$primary_street_address1,
                        'City'=>$primary_city,
                        'CountrySubDivisionCode'=>$primary_state,
                        'PostalCode'=>$primary_postal_code,
                ...
                }elseif(is_numeric($p1)&& is_numeric($quickbook_id)){
                    $rsl_vendor= $Object->updateQBVendorID_contactID($result["ID"],$quickbook_id);
                }
                //update qb_customer_id
                $rsl_customer='';
                if(!is_numeric($p1) && !is_numeric($p) && is_numeric($quickbook_id)){
                    $rsl_customer= $Object->updateQBVendor_contactID($result["ID"],$quickbook_id);
                }
                //
