...
$Object = new Claim();
    ...
    if(!$isAuth){
        ...
        $isAuth = $Object->validToken_logging($valid);
        //print_r($isAuth['email']);die();
        if($isAuth['AUTH']){
            $primary_email=trim($isAuth['email']);
            include_once './lib/class.login.php';
            $Object_login = new Login();
            if($isAuth['type']=="Admin" || $isAuth['type']=="Sales"){
                //$rsl = $Object_login->loginEmail(5,$primary_email,"","","","",$isAuth['type']);
                $rsl = $Object_login->loginEmailPass(5,$primary_email,"","","","","Sales");
            }/*else{
                $rsl = $Object_login->loginEmailPass(5,$primary_email,"","","","",$isAuth['type']);
            }*/
